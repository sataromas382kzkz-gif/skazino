...
function initPlinko() {
  ...
  const RESTITUTION = 0.15;    // потеря энергии при ударе о точку (мягче отскок)
  const STEER_STRENGTH = 3;    // слабое притяжение к слоту среди точек (ослаблено)
  const STEER_BELOW_PEGS = 10; // слабое притяжение ниже последнего ряда точек (ослаблено)
  const STEER_RESCUE = 20;     // аварийное притяжение при заклинивании (уменьшено)
  const MAX_SPEED = 400;       // ограничение скорости поменьше, для плавности падения
